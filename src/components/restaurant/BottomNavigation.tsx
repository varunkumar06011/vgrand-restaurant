...
const BottomNavigation: React.FC = () => {
  ...
  const handleWhatsApp = () => {
    const phone = '+919876543210'; // Restaurant WhatsApp number
      ...
      <nav className="fixed bottom-0 left-0 right-0 z-50 border-t border-border bg-background md:hidden">
        <div className="grid grid-cols-5 gap-1">
          ...
          <Link
            to="/menu"
            className={`flex flex-col items-center justify-center py-2 ${
              isActive('/menu') ? 'text-secondary' : 'text-muted-foreground'
            }`}
          >
          ...
          <button
            ...
            className="relative flex flex-col items-center justify-center py-2 text-muted-foreground"
          >
            <ShoppingCart className="h-5 w-5" />
            {totalItems > 0 && (
              <Badge
                variant="destructive"
                className="absolute right-1/4 top-1 h-5 w-5 rounded-full p-0 text-xs"
              >
                {totalItems}
              </Badge>
            )}
